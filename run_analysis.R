...
alldata$V1.1 <- revalue(alldata$V1.1, c("1"="walking", "2"="walking_upstairs", 
...
#    each activity and each subject; In my version, the subjects represent groups while the activities are
#    subgroups of these subjects, but you can also do it the other way around. 
...
#    within, then you should group it by making the activity the major group and subject the subgroup.
bySubAct <- group_by(alldata, subject, activity)
averages <- summarise_at(bySubAct, names(bySubAct[3:68]), mean)
...
# 6. Writing and reading a text file that contains the data set 'averages':
write.table(averages, "averages.txt", row.names=FALSE)
averagesread <- read.table("averages.txt", header=TRUE)

# 7. Saving all of the new variable names from the new data frame 'alldata' for the CodeBook:
